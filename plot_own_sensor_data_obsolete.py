...
for file in sensor_data_files:
    ...
    plt.savefig(os.path.join(output_dir, f'{base_filename}_obsolete.png'), dpi=dpi, bbox_inches='tight')
    plt.close()
    ...
    if expNumber == 28:
        plt.ylim(-10,20)
    plt.grid(True, which='both', linestyle='--', linewidth=1.5)
    plt.savefig(os.path.join(output_dir, f'{base_filename}_SHS_obsolete.png'), dpi=dpi, bbox_inches='tight')
    plt.close()
