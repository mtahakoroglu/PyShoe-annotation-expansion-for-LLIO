...
<p align="justify">At this point, before checking the performance of LSTM ZUPT detector on VICON training data, for all five experiments, we examined other ZUPT detectors' results to see if they are able to detect missed steps. Qualitately speaking, for missed ZUPT phase regions, other detectors such as SHOE and ARED were able to produce 1s in some cases. This observation gave us the idea of using ARED, SHOE or other ZUPT detectors as supplementary detectors to include the missed stride into the set of strides detected by LSTM ZUPT detector. However, we will not use this idea on own collected data. Instead we first visit VICON training dataset to see how LSTM ZUPT detector performs there. Then in case of missed strides in some experiments, mentioned supplementary ZUPT detectors can be used to correct for stride detections. As our ultimate goal is to extract a gait-driven system (stride & heading system) from VICON training dataset, which is a sampling-frequency driven system (sample-wise INS), by generating displacements and heading change values by using stride indexes on raw INS trajectory.</p>

<h3>Example Results (VICON training Data)</h3>
<p align="justify">Here, we showed some troublesome experiments to see the importance of correct stride detections. We ran the optimal ZUPT detectors, e.g., SHOE for experiment 4, with the optimal threshold values (these values are supplied by Wagstaff <i>et. al.</i>) in the structure of the mat files in VICON training dataof PyShoe dataset), we obtain the following figures.</p>
...
<p>As mentioned above, to detect the missed stride(s), we will exploit supplementary detectors such as VICON, ARED, MBGTD or AMVD. Generally VICON detector is able to generate ZUPT phases correctly so in many of the cases, we only use VICON ZUPT detector as the only supplementary detector. This process is annotating VICON training dataset manually to correct for errors in the training data. Later, generation of bipedal locomotion raining dataset is going to be completed by excluding crawling experiments.</p>
